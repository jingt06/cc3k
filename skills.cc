...
std::string DemoralizingShout:: use(player&p,floor&f){
	...
	for(int i=-2;i<=2;i++){
		for(int j=-2;j<=2;j++){
			...
			if((i!=0||j!=0)&&f.getObj(pr+i,pc+j)!=NULL){
				if(f.getObj(pr+i,pc+j)->getType()=='e'){
					int ld=f.getObj(pr+i,pc+j)->getAtk()/3;
					f.getObj(pr+i,pc+j)->changeAtk(-ld);
					action=action+"Demoralizing Shout decrease "+IntToString(ld)+" attack from "+f.getObj(pr+i,pc+j)->getName()+"\n";
					if(f.getObj(pr+i,pc+j)->is_dead()){
						int gold=f.getObj(pr+i,pc+j)->getTreasure();
						int exp=f.getObj(pr+i,pc+j)->getExp();
						p.incGold(gold);
...
string Slam::use(player& p,floor&f){
	if(cooldown>0) return IntToString(cooldown)+ " turns to Cool Down";
	cout<<"select direction(so,no,ea,we)"<<endl;
	string dir="";
	cooldown=cooldownRate;
	while(!(dir=="so"||dir=="no"||dir=="ea"||dir=="we")){
		cin>>dir;
	}
	string action="";
	int pr=p.getR();
